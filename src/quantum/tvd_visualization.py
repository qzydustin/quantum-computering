...
from .circuit_verification import verify_circuits
...
def load_results(
    ...
    """Load comparison/sequence results with optional in-memory generation pipeline."""
    tmp_path = Path(tmp_dir)
    comparison_path = tmp_path / 'circuit_comparison_results.json'
    sequence_path = tmp_path / 'sequence_analysis.json'
    resolved_circuits_dir = circuits_dir or str(tmp_path / 'circuits')

    # In-memory pipeline: reports -> sequence analysis + circuits -> verification
    if auto_generate and reports_dir:
        print("📦 Auto-generating analysis in memory from reports...")
        analysis = analyze_and_generate_circuits(
            data_dir=reports_dir,
            output_dir=str(tmp_path) if persist else None,
            persist=persist,
        )
        if not analysis:
            return None, None
        comparison_results = verify_circuits(
            config_file=config_file,
            circuits_dir=resolved_circuits_dir,
            output_file=str(comparison_path) if persist else None,
            reports_dir=reports_dir,
            auto_generate=False,
            circuit_entries=analysis.get('generated_circuits', []),
        )
        return comparison_results, analysis.get('sequence_analysis')
